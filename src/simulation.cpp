...
//    for(Shape s : voxels)
//    {
//        s.draw(m_normalsShader, true, s.alpha);
//    }
...
//    for(Shape s : voxels)
//    {
//        s.draw(m_normalsArrowShader, true, s.alpha);
//    }
    ...
    for(int i = 0; i < voxNum; i++)
    {
        for(int j = 0; j < voxNum; j++)
        {
            for(int k = 0; k < voxNum; k++)
            {
                ...
                {
                    Eigen::Vector3d l = {arrowData[n]+i,arrowData[n+1]+j,arrowData[n+2]+k};
                    pos.push_back(l);
                }
                ...
                STEMpos.push_back({0.0+i,0.0+j,0.0+k});
                STEMpos.push_back({0.0+i,-0.1+j,0.0+k});
                ...
                // Mesh rotation
                Eigen::AngleAxisd rollAngle(45.0, Eigen::Vector3d::UnitX());
                Eigen::AngleAxisd pitchAngle(0.0, Eigen::Vector3d::UnitY());
                Eigen::AngleAxisd yawAngle(0.0, Eigen::Vector3d::UnitZ());
                Eigen::Quaterniond q = yawAngle * pitchAngle * rollAngle;
                t.rotate(q);
