...
void Smoke::toggleARROWS()
{
    arrowsBOOL = !arrowsBOOL;
}
void Smoke::toggleDENSITY()
{
    densitiesBOOL = !densitiesBOOL;
}
void Smoke::toggleVOXELS()
{
    voxelsBOOL = !voxelsBOOL;
}

void Smoke::draw(Shader *shader, Shader *m_normalsShader, Shader *m_normalsArrowShader)
{
    shader->bind();
...
void Smoke::toggleWire()
{
    m_shape.toggleWireframe();
}

void Smoke::initGround()
{
    // Ground rotation
...
void Smoke::initSphere(std::shared_ptr<Grid> grid)
{
...
void Smoke::interaction(Vector3d dir)
{
    m_tetmesh->interaction(dir);
}

void Smoke::tilt_ground(float dir)
{
    tilt = tilt + dir;
